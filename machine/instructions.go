...
func printInstruction(instruction string, register uint16, value, pc uint32) {
	f := "%03d: %s - register r[%d] now contains 0x%08x\n"
	fmt.Printf(f, pc, instruction, register, value)
}
func printJump(pc uint32, instruction string, register uint32) {
	f := "%03d: %s     - jump to 0x%08X\n"
	fmt.Printf(f, pc, instruction, register)
}
func printBranch(pc uint32, instruction string, register uint32) {
	f := "%03d: %s- branch taken to 0x%08x\n"
	fmt.Printf(f, pc, instruction, register)
}
...
func addiu(m *Machine, inst uint32) {
	...
	sum := m.registers[s] + uint32(imm)
	printInstruction("addiu", t, sum, m.pc)
	m.registers[t] = sum
}
func slti(m *Machine, inst uint32) {
	...
	if s < imm {
		printInstruction("slti", tu, 1, m.pc)
		m.registers[tu] = 1
	} else {
		printInstruction("slti", tu, 0, m.pc)
		m.registers[tu] = 0
	}
}
func lui(m *Machine, inst uint32) {
	...
	val := imm << 16
	printInstruction("lui", tu, uint32(val), m.pc)
...
func xori(m *Machine, inst uint32) {
	...
	printInstruction("xori", tu, xoriVal, m.pc)
...
func mul(m *Machine, inst uint32) {
	...
	prod := s * t
	printInstruction("mul", du, prod, m.pc)
	m.registers[du] = prod
}
func lw(m *Machine, inst uint32) {
	...
	fmt.Printf("%03d: lw    - register r[%d] now contains 0x%08X\n", m.pc,
		t, value)
...
func sw(m *Machine, inst uint32) {
	...
	s, t, imm := binary.GetIFormat(inst)
	fmt.Printf("%03d: sw    - register r[%d] value stored in memory\n", m.pc, t)
...
func addu(m *Machine, inst uint32) {
	...
	printInstruction("addu", d, sum, m.pc)
	m.registers[d] = sum
}
func and(m *Machine, inst uint32) {
	...
	s, t := m.registers[su], m.registers[tu]
	andVal := s & t
	printInstruction("and", du, andVal, m.pc)
...
func jalr(m *Machine, inst uint32) {
	...
	m.registers[d] = m.pc
	printJump(m.pc, "jalr", m.registers[s])
	m.pc = m.registers[s]
}
func jr(m *Machine, inst uint32) {
	m.transferControl.jump++
	s, _, _, _, _ := binary.GetRFormat(inst)
	printJump(m.pc, "jr", m.registers[s])
	m.pc = m.registers[s]
}
func nor(m *Machine, inst uint32) {
	...
	s, t := m.registers[su], m.registers[tu]
	norVal := ^(s | t)
	printInstruction("nor", du, norVal, m.pc)
...
func or(m *Machine, inst uint32) {
	...
	s, t := m.registers[su], m.registers[tu]
	orVal := s | t
	printInstruction("or", du, orVal, m.pc)
...
func sll(m *Machine, inst uint32) {
	...
	t := m.registers[tu]
	sllVal := t << hu
	printInstruction("sll", du, sllVal, m.pc)
...
func sra(m *Machine, inst uint32) {
	...
	printInstruction("sll", du, sraVal, m.pc)
...
func srl(m *Machine, inst uint32) {
	...
	t := m.registers[tu]
	srlVal := t >> h
	printInstruction("srl", du, srlVal, m.pc)
...
func subu(m *Machine, inst uint32) {
	...
	printInstruction("subu", d, diff, m.pc)
	m.registers[d] = diff
}
func xor(m *Machine, inst uint32) {
	...
	s, t := m.registers[su], m.registers[tu]
	xorVal := s ^ t
	printInstruction("xor", du, xorVal, m.pc)
